...
std::optional<std::vector<at::Tensor>> te_general_grouped_gemm(
    ...
    size_t workspaceSize, float alpha, bool accumulate, bool use_split_accumulator, int math_sm_count);
...
void multi_tensor_ademamix_param_remainder_cuda(int chunk_size, at::Tensor noop_flag,
                                            std::vector<std::vector<at::Tensor>> tensor_lists,
                                            const float lr, const float beta1, const float beta2,
                                            const float beta3, const float alpha, const int normalize_alpha,
